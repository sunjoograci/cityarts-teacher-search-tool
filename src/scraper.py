...
def _looks_like_name(candidate: str) -> bool:
    """Reject obvious non-names: badges ("1 Year", "K-5"), and nav/menu
    labels and board-document titles that a naive word-count check accepts.
    Requires every word to look like a name token (Title Case, not ALL-CAPS)
    and rejects a small vocabulary of institutional nouns and punctuation.
    """
    ...
    if any(c in candidate for c in "&/:@|"):
        return False
    if _JUNK_PHRASE_RE.search(candidate):
        return False
    # Prefer the art_classifier's notion of "art-related" but keep older
    # _is_art_teacher check (compat) if available.
    try:
        if art_classifier.is_art_related(candidate):
            return False
    except Exception:
        pass
    if '_is_art_teacher' in globals() and _is_art_teacher(candidate):
        return False
...
# paragraph (long, with multiple sentences). Titles are expected to be
# concise.
...
if split and (art_classifier.is_art_related(split[1]) or _is_valid_art_title(split[1])):
...
# people's rows into one bogus record.
